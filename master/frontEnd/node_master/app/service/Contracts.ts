...
export default class Contracts extends Logger {
  ...
  async executeContract(name: string, method: any, params: any[], option: any) {
    if (!Contracts.web3) {
      Contracts.web3 = new Web3('http://localhost:8545');
    }
    let Contract;
    if (option.address && option.abi) {
      Contract = new Contracts.web3.eth.Contract(
        JSON.parse(option.abi as string),
        option.address
      );
    } else if (name) {
    ...
    const caller = (option.starter) || (await this.getAccounts())[0];
    await this.ctx.service.gethApi.unlockAccount(caller, option.password || '123456')();
    return await Contract
      .methods
      [method](...params)
      [option.type]({ from: caller })
      .then(res => {
        this.logObj(res.events.Print.returnValues);
        return {
          status: 'success',
          data: res.events.Print.returnValues
        };
      }).catch(err => {
  ...
  public async getBlockInfo() {
    ...
    if (Contracts.blockHeight < Number) {
      ...
      this.log(Contracts.blockHeight);
    }
