...
export default class ContractHandler extends Controller {
  ...
  public async execContractMethod() {
    const { ctx } = this;
    const { name, method, params, option } = ctx.request.body;
    if (!(name & method && params && option)) {
      ctx.body = {
        status: 'error',
        data: '缺少参数'
      };
    ...
    const result = await ctx.service.contracts.executeContract(name, method, params, option);
    ctx.body = result;
    return;
  }
